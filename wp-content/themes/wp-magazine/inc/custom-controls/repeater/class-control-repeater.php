...
if( ! class_exists( 'Wp_Magazine_Control_Repeater' ) ) {
    /**
     * Repeater control
     */
    class Wp_Magazine_Control_Repeater extends WP_Customize_Control {
    	...
    	public function __construct( $manager, $id, $args = array() ) {
    		...
    		$this->row_label = array(
    			'type' => 'text',
    			'value' => esc_attr__( 'row', 'wp-magazine' ),
    			'field' => false,
    		);
    		...
    		if ( empty( $this->button_label ) ) {
    			$this->button_label = sprintf( esc_attr__( 'Add new %s', 'wp-magazine' ), $this->row_label['value'] );
    		}
    	...
    	public function enqueue() {    
            ...
            wp_enqueue_script( 'wp-magazine-repeater', get_template_directory_uri() . '/inc/custom-controls/repeater/repeater.js', array( 'jquery', 'jquery-ui-core', 'jquery-ui-sortable' ), false, true );
    		wp_enqueue_style( 'wp-magazine-repeater', get_template_directory_uri() . '/inc/custom-controls/repeater/repeater.css', null );
    	}
    	...
    	protected function render_content() {
    		...
    		<?php if ( isset( $this->choices['limit'] ) ) : ?>
    			<p class="limit"><?php printf( esc_attr__( 'Limit: %s rows', 'wp-magazine' ), esc_html( $this->choices['limit'] ) ); ?></p>
    		<?php endif; ?>
    		...
    		<script type="text/html" class="customize-control-repeater-content">
    			...
    			<li class="repeater-row minimized" data-row="{{{ index }}}">
    				...
    				<div class="repeater-row-content">
    					<# _.each( data, function( field, i ) { #>
    						...
    						<div class="repeater-field repeater-field-{{{ field.type }}}">
    							...
    							<# } else if ( 'color' === field.type ) { #>
    						        ...
    						        <label>
    						            ...
    						            <# if ( field.description ) { #>
    						                <span class="description customize-control-description">{{{ field.description }}}</span>
    						            <# } #>
    						            <input class="color-picker-hex" type="text" maxlength="7" placeholder="<?php echo esc_attr__( 'Hex Value', 'wp-magazine' ); ?>"  value="{{{ field.default }}}" data-field="{{{ field.id }}}" {{ defaultValue }} />
    							...
    							<# } else if ( field.type === 'image' || field.type === 'cropped_image' ) { #>
    								...
    								<figure class="wp-magazine-image-attachment" data-placeholder="<?php echo esc_attr( 'No Image Selected', 'wp-magazine' ); ?>" >
    									<# if ( field.default ) { #>
    										<# var defaultImageURL = ( field.default.url ) ? field.default.url : field.default; #>
    										<img src="{{{ defaultImageURL }}}">
    									<# } else { #>
    										<?php echo esc_attr( 'No Image Selected', 'wp-magazine' ); ?>
    									<# } #>
    								</figure>
    
    								<div class="actions">
    									<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"><?php esc_html_e( 'Remove', 'wp-magazine' ); ?></button>
    									<button type="button" class="button upload-button" data-label=" <?php echo esc_attr( 'Add Image', 'wp-magazine' ); ?>" data-alt-label="<?php echo esc_attr( 'Change Image', 'wp-magazine' ); ?>" >
    										<# if ( field.default ) { #>
    											<?php echo esc_attr( 'Change Image', 'wp-magazine' ); ?>
    										<# } else { #>
    											<?php echo esc_attr( 'Add Image', 'wp-magazine' ); ?>
    										<# } #>
    									</button>
    									<# if ( field.default.id ) { #>
    							...
    							<# } else if ( field.type === 'upload' ) { #>
    								...
    								<figure class="wp-magazine-file-attachment" data-placeholder="<?php echo esc_attr( 'No File Selected', 'wp-magazine' ); ?>" >
    									<# if ( field.default ) { #>
    									...
    									<# } else { #>
    										<?php echo esc_attr( 'No File Selected', 'wp-magazine' ); ?>
    									<# } #>
    								</figure>
    
    								<div class="actions">
    									<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"></button>
    									<button type="button" class="button upload-button" data-label="<?php echo esc_attr( 'Add File', 'wp-magazine' ); ?>" data-alt-label="<?php echo esc_attr( 'Change File', 'wp-magazine' ); ?>" >
    										<# if ( field.default ) { #>
    											<?php echo esc_attr( 'Change File', 'wp-magazine' ); ?>
    										<# } else { #>
    											<?php echo esc_attr( 'Add File', 'wp-magazine' ); ?>
    										<# } #>
    									</button>
    									<# if ( field.default.id ) { #>
    										<input type="hidden" class="hidden-field" value="{{{ field.default.id }}}" data-field="{{{ field.id }}}" >
    					...
    					<# }); #>
    					<button type="button" class="button-link repeater-row-remove"><?php esc_html_e( 'Remove', 'wp-magazine' ); ?></button>
    				</div>
    			</li>
    		</script>
    		<?php
